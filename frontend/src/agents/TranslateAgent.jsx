...
const TranslateAgent = () => {
  ...
  const messagesEndRef = useRef(null); // 👈 anchor ref for scroll-to-bottom
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API_BASE_URL}/translate/content/${userId}`, {
  ...
  // 👇 Add after your existing useEffect blocks
  useEffect(() => {
    scrollToBottom();
  }, [messages]);
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await axios.post(
        `${API_BASE_URL}/translate/content/${userId}`,
        { prompt: userInput },
        { headers: { Authorization: `${token}` } }
      );
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col items-center">
      <h1 className="text-white text-lg font-semibold mt-4">Lingua-Ai</h1>
