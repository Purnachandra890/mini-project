...
const ScriptAgent = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API_BASE_URL}/script/content/${userId}`, {
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await axios.post(
        `${API_BASE_URL}/script/content/${userId}`,
        { prompt: userInput },
        { headers: { Authorization: `${token}` } }
      );
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col items-center">
      ...
      <div className="flex-1 w-full max-w-2xl px-4 py-6 space-y-4 overflow-y-auto">
        ...
        <div ref={messagesEndRef} /> {/* 👈 anchor for scroll */}
      </div>
      ...
      <div className="w-full max-w-2xl px-4 py-4 sticky bottom-0 bg-black space-y-2">
        ...
        <div className="bg-zinc-900 text-white rounded-3xl px-4 py-0.1">
          <textarea
            placeholder="Describe your script idea... (press Enter to send)"
